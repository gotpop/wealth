...
export const TabButton = ({ tabItem, handleClick, activeTabData }: Props) => {
  ...
  return (
    <div className="basis-60 shrink grow flex flex-col items-center p-4">
      <button className="flex font-body font-bold tracking-wide mb-2 text-brand-grey-6 text-[14px]" onClick={() => handleClick(tabItem)}>
        {title}
      </button>
      {activeTabData.title === title ? <span className="inline-flex min-w-[50%] border-b-4 border-secondary"></span> : null}
    </div>
  )
}
